<?php
	...
	if(isset($_GET["tag"])) {
		...
		$conn = connDB();
		$stmt = $conn->prepare("select * from Product where type = '1' and tag = :tag order by updatetime DESC limit 10");
		...
		$conn = connDB();
		$stmt = $conn->prepare("select * from Product where type = '1' order by updatetime DESC limit 10");
		$stmt->execute();
		$result = $stmt->fetchAll(PDO::FETCH_ASSOC);
